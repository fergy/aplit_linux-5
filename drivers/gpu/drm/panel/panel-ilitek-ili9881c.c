...
static const struct drm_display_mode sc20_default_mode = {
	.clock		= 62000,
	.vrefresh	= 60,
...
static int ili9881c_get_modes(struct drm_panel *panel)
{
	...
	mode = drm_mode_duplicate(panel->drm, &sc20_default_mode);
	if (!mode) {
		dev_err(&ctx->dsi->dev, "failed to add mode %ux%ux@%u\n",
			sc20_default_mode.hdisplay,
			sc20_default_mode.vdisplay,
			sc20_default_mode.vrefresh);
		return -ENOMEM;
	}
...
static const struct of_device_id ili9881c_of_match[] = {
	{ .compatible = "ilitek,ili9881c", "qcom,msm8909" },
	{ }
};
