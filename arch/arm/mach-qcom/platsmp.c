...
#define MSM8909_BASE	0xb088000

static int msm8909_release_secondary(unsigned int cpu)
{
	void *base_ptr = ioremap_nocache(MSM8909_BASE + (cpu * 0x10000), SZ_4K);
	if (!base_ptr)
		return -ENODEV;
...
{
	return qcom_boot_secondary(cpu, msm8909_release_secondary);
}
...
static const struct smp_operations smp_msm8909_ops __initconst = {
	.smp_prepare_cpus	= qcom_smp_prepare_cpus,
	.smp_secondary_init	= qcom_secondary_init,
...
CPU_METHOD_OF_DECLARE(qcom_smp_msm8909, "qcom,acc-msm8909", &smp_msm8909_ops);
